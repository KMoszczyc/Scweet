...
from .utils import init_driver, get_last_date_from_csv, log_search_page, keep_scroling, dowload_images, log_in, get_retry_button, handle_retry_timout_screen, is_cookie_expired, load_netscape_cookies

def refresh_cookies(driver, cookies):
    driver.get("http")
...
def scrape(since, until=None, words=None, to_account=None, from_account=None, mention_account=None, interval=5, lang=None,
          ...
          geocode=None, minreplies=None, minlikes=None, minretweets=None, cookies_dir=None, cookie_id=None, outputs_dir="outputs"):
    """
    ...
    # ------------------------- Variables : 
    # header of csv
    cookies = load_netscape_cookies(cookies_dir)
    print("Available cookies: ", len(cookies))
    if not cookies:
        print("No cookies found")
        return
    scraping_accounts_num = len(cookies)
    current_cookie_id = cookie_id if cookie_id is not None else 0
    cookies_timeout_state = {i: False for i in range(scraping_accounts_num)}
    ...
    log_in(driver, '.env', cookies[current_cookie_id], wait=5)
    ...
    with open(path, write_mode, newline='', encoding='utf-8') as f:
        ...
        while until_local <= datetime.datetime.strptime(until, '%Y-%m-%d'):
            ...
            if is_cookie_expired(driver):
                print("================== Attention!! Cookie expired, place replace it with a new one! ==================")
                return
            ...
            cookies_timeout_state, current_cookie_id, cookie_swap = handle_retry_timout_screen(driver, path, cookies, cookies_timeout_state, current_cookie_id)

            # number of logged pages (refresh each <interval>)
            refresh += 1
            last_position = driver.execute_script("return window.pageYOffset;")
            # should we keep scrolling ?
            scrolling = False if cookie_swap else True # dont scroll after cookie swap cuz its set now for twitter home page
            ...
            if not df.empty:
                path = f'{outputs_dir}/{from_account}_{until_local.strftime('%Y-%m-%d')}.parquet'
                df.to_parquet(path)
            else:
                print(f'No tweets found this day: {until_local.strftime('%Y-%m-%d')}')
